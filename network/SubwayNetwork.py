...
#     def __init__(self, db_path: str = 'subway.db'):
#         """
...
#     def update_from_live_feeds(self, verbose: bool = True) -> Dict[str, int]:
#         """
#         Update edge weights based on live MTA GTFS-RT feeds.
#         Edges not present in the live feed are removed from memory (indicating service disruption).
...
#         # Track which edges we've seen in the live data
#         live_edges_seen = set()
...
#                                 # Update edge weight if it exists
#                                 self._update_edge_weight(prev_stop, stop_id, travel_time)
#                                 live_edges_seen.add((prev_stop, stop_id))
...
#         # Remove edges that weren't seen in live data (service disruptions)
#         stats['edges_removed'] = self._remove_inactive_edges(live_edges_seen)
        
#         self.live_updates_timestamp = datetime.now()
...
#     def _remove_inactive_edges(self, live_edges_seen: Set[Tuple[str, str]]) -> int:
#         """
#         Remove edges that weren't seen in the live feed (only for 'run' type edges).
#         Preserves intra-station transfer edges.
...
#                 # Only keep run edges that were seen in live data
#                 elif (source, edge['target']) in live_edges_seen:
#                     updated_edges.append(edge)
#                 else:
#                     removed_count += 1
...
#             else:
#                 # Remove source entirely if no edges remain
...
#     def find_route(self, start: str, goal: str, use_live_data: bool = True) -> Dict:
#         """
...
#             self.update_from_live_feeds(verbose=False)
...
#     if disrupted:
#         print(f"\n Found {len(disrupted)} disrupted edges")
